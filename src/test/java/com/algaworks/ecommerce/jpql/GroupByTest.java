...
//      Total de vendas por mês
        /*String jpql = "select concat(year(p.dataCriacao), '/', function('monthname', p.dataCriacao)), " +
                "sum(p.total) from Pedido p group by year(p.dataCriacao), month(p.dataCriacao)" ;*/
...
//      Total de vendas por cliente
        /*String jpql = "select cl.nome, sum(ip.precoProduto) from ItemPedido ip " +
                " join ip.pedido pd " +
                " join pd.cliente cl " +
                " group by cl.id";*/

//      Total de vendas por dia e por categoria
        String jpql = "select ct.nome, pd.dataCriacao from ItemPedido ip " +
                " join ip.pedido pd " +
                " join ip.produto pr " +
                " join pr.categorias ct " +
                " group by pd.dataCriacao, ct.nome";
...
//      Total de vendas por mês usando where filtrando por resultados deste ano
        /*String jpql = "select concat(year(p.dataCriacao), '/', function('monthname', p.dataCriacao)), " +
